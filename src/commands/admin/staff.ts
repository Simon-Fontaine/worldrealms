...
module.exports = {
	...
	async autocomplete(interaction: AutocompleteInteraction) {
		...
		return await interaction.respond(
			roles.map((role) => ({ name: role._id, value: role._id }))
		);
	},
	async execute(interaction: ChatInputCommandInteraction) {
		...
		try {
			await member.roles.remove(roles.map((role) => role.role_id));
		} catch (ignored) {}

		if (!role) {
			const memberData = await userSchema.findOneAndDelete({ _id: user.id });
			if (!memberData) {
				return await interaction.editReply({
